        ...
        public void ExecuteTransaction(ExchangeAction protocolHandler, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < ServiceEndpoint.RetryCountLimit && retryAllowed && watch.Elapsed < ServiceEndpoint.ConnectionErrorRetryTimeout; i++)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        connection = connectionManager.AcquireConnection(exchangeProtocolBuilder, new TcpConnectionFactory(clientCertificate, asyncHalibutFeature, halibutTimeoutsAndLimits, streamFactory), ServiceEndpoint, log, cancellationToken);
                ...
                try
                {
                    ...
                    try
                    {
                        connection = await connectionManager.AcquireConnectionAsync(
                            ...
                            new TcpConnectionFactory(clientCertificate, asyncHalibutFeature, halibutTimeoutsAndLimits, streamFactory), 
                            ServiceEndpoint,
                            log, 
