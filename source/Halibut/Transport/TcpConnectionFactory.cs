...
namespace Halibut.Transport
{
    public class TcpConnectionFactory : IConnectionFactory
    {
        ...
        public IConnection EstablishNewConnection(ExchangeProtocolBuilder exchangeProtocolBuilder, ServiceEndPoint serviceEndpoint, HalibutTimeouts halibutTimeouts, ILog log)
        {
            return EstablishNewConnection(exchangeProtocolBuilder, serviceEndpoint, halibutTimeouts, log, CancellationToken.None);
        }

        public IConnection EstablishNewConnection(ExchangeProtocolBuilder exchangeProtocolBuilder, ServiceEndPoint serviceEndpoint, HalibutTimeouts halibutTimeouts, ILog log, CancellationToken cancellationToken)
        {
            ...
            var client = CreateConnectedTcpClient(serviceEndpoint, halibutTimeouts, log, cancellationToken);
            ...
            return new SecureConnection(client, ssl, exchangeProtocolBuilder, halibutTimeouts, log);
        }

        internal static TcpClient CreateConnectedTcpClient(ServiceEndPoint endPoint, HalibutTimeouts halibutTimeouts, ILog log, CancellationToken cancellationToken)
        {
            TcpClient client;
            if (endPoint.Proxy == null)
            {
                client = CreateTcpClient(halibutTimeouts);
            ...
            else
            {
                ...
                client = new ProxyClientFactory()
                    ...
                    .WithTcpClientFactory(() => CreateTcpClient(halibutTimeouts))
                    .CreateConnection(endPoint.BaseUri.Host, endPoint.BaseUri.Port, endPoint.TcpClientConnectTimeout, cancellationToken);
            }
            return client;
        }

        internal static TcpClient CreateTcpClient(HalibutTimeouts halibutTimeouts)
        {
            var addressFamily = Socket.OSSupportsIPv6
                ? AddressFamily.InterNetworkV6
                : AddressFamily.InterNetwork;

            return CreateTcpClient(addressFamily, halibutTimeouts);
        }

        internal static TcpClient CreateTcpClient(AddressFamily addressFamily, HalibutTimeouts halibutTimeouts)
        {
            var client = new TcpClient(addressFamily)
            {
                SendTimeout = (int)halibutTimeouts.TcpClientSendTimeout.TotalMilliseconds,
                ReceiveTimeout = (int)halibutTimeouts.TcpClientReceiveTimeout.TotalMilliseconds
            };
