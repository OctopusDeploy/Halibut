...
{
    public class AsyncHalibutProxy : DispatchProxyAsync
    {
        Func<RequestMessage, CancellationToken, ResponseMessage> syncMessageRouter;
        Func<RequestMessage, CancellationToken, Task<ResponseMessage>> asyncMessageRouter;
        Type contractType;
        ServiceEndPoint endPoint;
        CancellationToken cancellationToken;
        long callId;

        public AsyncHalibutProxy()
        {
        ...
        public void Initialise(
            ...
            CancellationToken cancellationToken)
        {
            this.syncMessageRouter = syncMessageRouter;
            this.asyncMessageRouter = asyncMessageRouter;
        ...
        public override object Invoke(MethodInfo method, object[] args)
        {
            ...
            var response = syncMessageRouter(request, cancellationToken);
        ...
        {
            var request = CreateRequest(method, args);
            var response = await asyncMessageRouter(request, cancellationToken);
            EnsureNotError(response);
        }
        ...
        public override async Task<T> InvokeAsyncT<T>(MethodInfo method, object[] args)
        {
            ...
            var returnType = method.ReturnType;
            // var returnTypeGenericArgument = method.ReturnType.GetGenericArguments().FirstOrDefault();
            // TODO: this will undoubtedly still need some work.
            if (result != null && returnType != typeof(void) && !returnType.IsAssignableFrom(result.GetType()))
            {
                result = Convert.ChangeType(result, returnType);
            }
        ...
        RequestMessage CreateRequest(MethodInfo method, object[] args)
        {
    ...
    public abstract class DispatchProxyAsync
    {
        ...
        {
            var proxy = (T)AsyncDispatchProxyGenerator.CreateProxyInstance(typeof(TProxy), typeof(T));
            return proxy;
        }
    ...
    //
    static class AsyncDispatchProxyGenerator
    {
        const int InvokeActionFieldAndCtorParameterIndex = 0;
