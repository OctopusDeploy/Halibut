    ...
    public class NextAndProceedControlMessageTimeoutsFixture : BaseTest
    {
        ...
        public async Task PauseOnPollingTentacleSendingNextControlMessage_ShouldNotHangForEver(ClientAndServiceTestCase clientAndServiceTestCase)
        {
                       ...
                       .Build(CancellationToken))
            {
                ...
                sw.Elapsed.Should().BeGreaterThanOrEqualTo(clientAndService.Service.TimeoutsAndLimits.TcpClientHeartbeatReceiveTimeout - TimeSpan.FromSeconds(2)) // -2s since tentacle will begin its countdown on the read which may start just after
                                                                                                                                     // the response to the 'Second last message' is put back into the queue.
                    .And.BeLessThan(clientAndService.Service.TimeoutsAndLimits.TcpClientReceiveTimeout, "Service should not be using this timeout to detect the control message is not coming back, it should use the shorter one.");
            }
        }
    }
}