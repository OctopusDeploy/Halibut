...
| `--user 1001:1001` | Runs Redis as a non-root user. `1001` is just an example, any non-root user works. We override the image's entrypoint (see below), and that entrypoint is what normally switches to a non-root user, so without `--user` Redis runs as root. | No, but we recommend not running as root. |
...
| `--entrypoint redis-server` | Starts `redis-server` directly. The official image's entrypoint loads the Redis 8 modules (JSON, search etc.), which the queue does not use. | No, but recommended so Redis runs only what the queue needs. |
| `redis:8.0.3` | The Redis version the queue is tested against. | No, but recommended. |
| `/etc/redis/redis.conf` | Tells `redis-server` to use the mounted config file. | Yes. |
| `--requirepass halibut-local-redis` | Requires clients to authenticate. `halibut-local-redis` is the password the tests look for by default. | Yes for the tests. |
